...
def get_notification_link(extra_data:dict|None, role:str):
    try:
        return reverse(extra_data.get(f'{role}_link'), kwargs=extra_data.get(f'{role}_link_kwargs'))
    except Exception as e:
        return '#'

def create_notification(actor, verb, content_object=None, child=None, recipients=None, extra_data:dict|None=None, notification_type=Notification.NotificationTypes.INFO):
    """
    ...
    Args:
        ...
        extra_data (dict, optional): Additional context data for the notification. (Like a link to notifications).
        notification_type (str): The type of notification (INFO, WARNING, ERROR).

    Returns:
        Notification: The created notification instance and links to different users
    """

    # Pass links using django namespaces in extra data  to create different different links to different users.
    notification_links = {
    'company_link': get_notification_link(extra_data=extra_data, role='company'),
    'tutor_link': get_notification_link(extra_data=extra_data, role='tutor'),
    'parent_link': get_notification_link(extra_data=extra_data, role='parent'),
    }
