...
from .models import SessionRate, Payment, TutorPayRate, TutorPayments
...
class TutorPaymentsAdmin(admin.ModelAdmin):
    # Display fields in the list view
    list_display = ('tx_ref', 'child_link', 'tutor_link', 'amount', 'status', 'formatted_timestamp')
    list_filter = ('status', 'timestamp')
    search_fields = ('tx_ref', 'child__first_name', 'child__last_name', 'tutor__first_name', 'tutor__last_name')
    ordering = ('-timestamp',)
    autocomplete_fields = ('child', 'tutor')
    filter_horizontal = ('sessions',)
    readonly_fields = ('timestamp',)

    # Custom links for child and tutor
    def child_link(self, obj):
        if obj.child:
            app_label = obj.child._meta.app_label
            model_name = obj.child._meta.model_name
    ...
    def get_form(self, request, obj=None, **kwargs):
        form = super().get_form(request, obj, **kwargs)
        form.base_fields['child'].help_text = "Select the child this payment is for."
        form.base_fields['tutor'].help_text = "Select the tutor receiving this payment."
        form.base_fields['amount'].help_text = "Enter the payment amount (e.g., 1200.00 ETB)."
        form.base_fields['status'].help_text = "Set the payment status (e.g., Pending, Success, Failed)."
        form.base_fields['sessions'].help_text = "Select the sessions covered by this payment."
        return form