...
class PaymentListView(CompanyRequiredMixin, ListView):
    model = Payment
    template_name = 'company/payments/parent_payment_list.html'
...
class TutorPaymentListView(CompanyRequiredMixin, ListView):
    model = TutorPayments
    template_name = 'company/payments/tutor_payment_list.html'
    context_object_name = 'payments'
    paginate_by = 25
    annotate_fields = {
        'total_sessions': Count('sessions', distinct=True),
    ...
    def _get_base_queryset(self):
        """Base queryset with company restriction and select_related optimizations."""
        return TutorPayments.objects.filter(
